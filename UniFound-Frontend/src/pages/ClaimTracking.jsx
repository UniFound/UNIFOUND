    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans selection:bg-blue-100">
      ...
      <div className="max-w-[1400px] mx-auto pt-24 pb-12 px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* LEFT: Stepper & Handover (Main Area) */}
            ...
            <div className="bg-white rounded-[2.5rem] p-8 md:p-10 border border-slate-100 shadow-sm relative overflow-hidden">
               ...
               <div className="relative space-y-12">
                  ...
                  <CompactStep 
                    ...
                    desc="Admin team is reviewing provided images and descriptions." 
                    status={status === "verified" ? "completed" : status === "rejected" ? "error" : "active"}
                  />

                  <CompactStep 
                    title={status === "rejected" ? "Claim Rejected" : "Final Handover"} 
                    desc={status === "verified" ? "Verification passed! View collection details below." : "Decision pending from campus administration."} 
                    status={status === "verified" ? "completed" : status === "rejected" ? "error" : "pending"}
                    time={status === "verified" ? claim.updatedAt : null}
                  />
               </div>
            </div>
  ...
  return (
    <div className="relative pl-14 group">
      <div className={`absolute left-0 top-0 w-10 h-10 rounded-2xl border-4 border-white shadow-sm flex items-center justify-center z-10 transition-all duration-500 
        ${isCompleted ? "bg-emerald-500 shadow-emerald-100" : isError ? "bg-rose-500 shadow-rose-100" : isActive ? "bg-blue-600 animate-pulse" : "bg-slate-100"}`}>
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
           <h4 className={`text-sm font-black tracking-tight uppercase ${isCompleted ? "text-slate-900" : isActive ? "text-blue-600" : "text-slate-300"}`}>{title}</h4>
