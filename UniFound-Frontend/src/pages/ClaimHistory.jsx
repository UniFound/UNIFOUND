    ...
    <div className="h-screen w-screen bg-[#F8FAFC] selection:bg-blue-100 text-slate-600 font-sans overflow-hidden flex flex-col">
      ...
      <div className="flex-1 flex flex-col xl:flex-row pt-20 overflow-hidden">
        ...
        <div className="w-full xl:w-[380px] xl:h-full bg-white/70 backdrop-blur-xl border-r border-slate-200/60 p-6 md:p-8 flex flex-col justify-between overflow-y-auto">
          <div className="space-y-6">
            <div className="flex items-center justify-between gap-4">
              <button onClick={() => navigate(-1)} className="flex items-center gap-2.5 text-slate-500 hover:text-blue-600 transition-all group font-bold text-xs">
              ...
              <button onClick={fetchData} disabled={loading} className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 text-xs font-bold disabled:opacity-50">
            ...
            <div className="space-y-3">
              ...
              <button onClick={() => setActiveTab("my-claims")} className={`w-full flex items-center justify-between p-4 rounded-xl transition-all duration-300 ${activeTab === "my-claims" ? "bg-slate-900 text-white shadow-xl shadow-slate-900/10" : "bg-white text-slate-600 hover:bg-slate-50 border border-slate-100"}`}>
                <div className="flex items-center gap-3">
              ...
              <button onClick={() => setActiveTab("my-items")} className={`w-full flex items-center justify-between p-4 rounded-xl transition-all duration-300 ${activeTab === "my-items" ? "bg-slate-900 text-white shadow-xl shadow-slate-900/10" : "bg-white text-slate-600 hover:bg-slate-50 border border-slate-100"}`}>
                <div className="flex items-center gap-3">
        ...
        <div className="flex-1 h-full overflow-y-auto bg-slate-50/50 p-6 md:p-8 xl:p-10">
            ...
            claims.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-6">
                {claims.map((claim) => {
                  ...
                  return (
                    <div key={claim._id} className={`group bg-white border border-slate-200/60 rounded-2xl p-6 transition-all duration-500 flex flex-col justify-between min-h-[290px] ${claimStatus === "rejected" ? "opacity-75" : "hover:shadow-2xl hover:shadow-slate-200/50 hover:-translate-y-1.5"}`}>
                      <div>
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg font-bold text-[10px] tracking-widest ${theme.badge}`}>
                             {claimStatus === "approved" || claimStatus === "verified" ? <CheckCircle2 size={10} /> : claimStatus === "rejected" ? <XCircle size={10} /> : <Clock size={10} />}
                        ...
                        <div className="flex gap-4 items-start mb-5">
                          ...
                          <div>
                            <h3 className="text-base font-bold text-slate-900 tracking-tight group-hover:text-blue-600 transition-colors line-clamp-2">{itemInfo.title || "Item Claimed"}</h3>
                      ...
                      {claimStatus === "approved" || claimStatus === "verified" ? (
                        <div className="flex flex-col gap-2">
                          <button onClick={() => navigate(`/chat/${claim.claimId}`)} className="w-full py-2.5 bg-blue-600 text-white rounded-xl font-bold text-xs tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 flex items-center justify-center gap-2">
                            <MessageSquare size={12} /> CHAT WITH FINDER <ChevronRight size={12} />
                          </button>
                          <button onClick={() => navigate(`/claim-details/${claim.claimId}`)} className="w-full py-2.5 bg-white border border-slate-200 text-slate-700 rounded-xl font-bold text-xs tracking-widest transition-all flex items-center justify-center gap-2 hover:bg-slate-50 hover:text-slate-700 hover:border-slate-300">
                            VIEW LOGS <ChevronRight size={12} />
                          </button>
                        </div>
                      ) : (
                        <button onClick={() => navigate(`/claim-details/${claim.claimId}`)} className="w-full py-3 bg-white border border-slate-200 text-slate-700 group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 rounded-xl font-bold text-xs tracking-widest transition-all duration-300 flex items-center justify-center gap-2">
                          VIEW LOGS <ChevronRight size={12} />
                        </button>
                      )}
                    </div>
          ...
          {/* 2. MY REPORTED ITEMS GRID (Lost Item modifications here) */}
          {activeTab === "my-items" && (
            reportedItems.length === 0 ? (
              <div className="flex flex-col items-center justify-center text-center h-[50vh] bg-white/80 backdrop-blur border border-slate-200/60 rounded-3xl p-12">
                ...
                <button onClick={() => navigate("/report")} className="px-4 py-2.5 bg-blue-600 text-white text-xs font-bold rounded-xl flex items-center gap-1 hover:bg-blue-700 shadow-lg shadow-blue-100 transition-all">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-6">
                {reportedItems.map((item) => {
                  ...
                  // Mock match count - Backend එකෙන් එනවා නම් item.matchCount වගේ එකක් පාවිච්චි කරන්න
                  const matchCount = 3; 

                  return (
                    <div key={item._id} className="group relative bg-white border border-slate-200/60 rounded-2xl p-6 transition-all duration-500 hover:shadow-2xl hover:shadow-slate-200/50 hover:-translate-y-1.5 flex flex-col justify-between min-h-[290px]">
                      
                      {/* 🔥 Potential Matches Badge (Only for Lost Items) */}
                      {isLost && matchCount > 0 && (
                        <div className="absolute -top-2 -right-2 z-10 animate-pulse">
                          <div className="bg-orange-500 text-white text-[9px] font-black px-2.5 py-1 rounded-lg shadow-lg flex items-center gap-1 border-2 border-white uppercase tracking-tighter">
                            <Flame size={10} fill="white" /> {matchCount} Matches
                          </div>
                        </div>
                      )}
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg font-bold text-[10px] tracking-widest ${theme.badge}`}>
                             {item.status === 'found' || item.status === 'returned' ? <CheckCircle2 size={10} /> : <XCircle size={10} />}
                        ...
                        <div className="flex gap-4 items-start mb-5">
                          ...
                          <div>
                            <h3 className="text-base font-bold text-slate-900 tracking-tight group-hover:text-blue-600 transition-colors line-clamp-2">{item.title}</h3>
                      ...
                      <div className="flex items-center gap-2">
                        <button 
                          ...
                          className={`flex-1 py-3 rounded-xl font-bold text-xs tracking-widest transition-all duration-300 flex items-center justify-center gap-2 ${
                            isLost && matchCount > 0 
                            ? "bg-blue-600 text-white shadow-lg shadow-blue-100 hover:bg-blue-700" 
                            : "bg-white border border-slate-200 text-slate-700 hover:bg-blue-600 hover:text-white hover:border-blue-600"
                          }`}
                        >
                          {isLost && matchCount > 0 ? "VIEW MATCHES" : "VIEW ITEM"}
                          <ChevronRight size={12} />
                        </button>

                        {/* 🔗 Share Button (Only for Lost Items) */}
                        {isLost && (
