...
export default function AdminDashboard() {
  // State initialization from API logic
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (usersData && Array.isArray(usersData)) {
          setUserCount(usersData.length.toLocaleString());
        } else if (usersData.users && Array.isArray(usersData.users)) {
          setUserCount(usersData.users.length.toLocaleString());
        }
        ...
        if (itemsData && Array.isArray(itemsData)) {
          setFoundCount(itemsData.length.toLocaleString());
        } else if (itemsData.items && Array.isArray(itemsData.items)) {
          setFoundCount(itemsData.items.length.toLocaleString());
        }
        ...
        if (claimsData && Array.isArray(claimsData)) {
          setClaimCount(claimsData.length.toLocaleString());
        } else if (claimsData.claims && Array.isArray(claimsData.claims)) {
          setClaimCount(claimsData.claims.length.toLocaleString());
        }
        ...
        if (ticketsData && Array.isArray(ticketsData)) {
          setTicketCount(ticketsData.length.toLocaleString());
        } else if (ticketsData.tickets && Array.isArray(ticketsData.tickets)) {
          setTicketCount(ticketsData.tickets.length.toLocaleString());
        }
      } catch (error) {
  ...
  // Chart data from test-fixed branch
  const weeklyData = [
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFF] font-['Inter',_-apple-system,_.SFNSText-Regular,'Segoe_UI','Helvetica_Neu',sans-serif] text-[#1E293B]">
      <Sidebar />
                ...
                <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 bg-white rounded-[32px] p-8 shadow-sm border border-slate-100/50 relative overflow-hidden flex flex-col">
                      ...
                      {/* මෙහිදී PieChart සහ Bar Chart දෙකම අවශ්‍ය නම් තබාගත හැක. දැනට Bar Chart එක පහතින් ඇත. */}
